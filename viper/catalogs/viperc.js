...
module.exports = (text, type) => {
  switch (type) {
		case "warn":
			process.stderr.write(color(`viper - `) + text + '\n');
			break;
		case "error":
			process.stderr.write(chalk.bold.hex("#ff0000").bold(`viper - `) + text + '\n');
			break;
		case "load":
...
module.exports.warn = (text, type) => {
	process.stderr.write(crayon(`viper - `) + text + '\n');
};

module.exports.commands = (text, type) => {
	process.stderr.write(blu(`viper - `) + text + '\n');
};
...
module.exports.loader = (data, option) => {
	switch (option) {
		...
		case "error":
			...
			break;
		default:
			process.stderr.write(blu(`viper - `) + data + '\n');
			break;
	}
}