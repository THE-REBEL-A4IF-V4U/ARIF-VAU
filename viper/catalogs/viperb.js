...
global.utils = require("./viperd.js");
global.loading = require("./viperc.js");
...
var ryukoValue;
try {
  global.client.ryukoPath = join(global.client.mainPath, "../configs/viper.json");
...
var configValue;
try {
  global.client.configPath = join(global.client.mainPath, "../../viper.json");
  configValue = require(global.client.configPath);
  logger.loader(`deploying ${chalk.blueBright('viper')} file`);
} catch (e) {
  return logger.loader(`cant read ${chalk.blueBright('viper')} file`, "error");
}
try {
  for (const key in configValue) global.config[key] = configValue[key];
  logger.loader(`deployed ${chalk.blueBright('viper')} file`);
} catch (e) {
  return logger.loader(`can't deploy ${chalk.blueBright('viper')} file`, "error")
}
...
try {
  var appStateFile = resolve(join(global.client.mainPath, "../../viperstate.json"));
  ...
  logger.loader(`deployed ${chalk.blueBright('viperstate')} file`)
} catch (e) {
  return logger.error(`can't read ${chalk.blueBright('viperstate')} file`)
}
function onBot({ models: botModel }) {
  ...
  console.clear();
  console.log(chalk.blue(`DEPLOYING VIPER COMMANDS\n`));
        for (const command of listCommand) {
          try {
            ...
            if (!config?.category) {
              try {
                throw new Error(`viper - ${command} category is not in the correct format or empty`);
              } catch (error) {
            ...
            if (!config?.hasOwnProperty('prefix')) {
              console.log(`viper -`, chalk.hex("#ff0000")(command) + ` does not have the "prefix" property.`);
              continue;
            }

            if (global.client.commands.has(config.name || '')) {
              console.log(chalk.red(`viper - ${chalk.hex("#FFFF00")(command)} module is already deployed.`));
              continue;
            }
            ...
            if (envConfig) {
              ...
              var ryukoPath = require('../configs/viper.json');
            ...
            if (module.onLoad) {
              ...
              try {
                ...
                throw new Error(errorMessage, 'viper');
              }
            }
            ...
            try {
              global.loading(`${crayon(``)}successfully deployed ${chalk.blueBright(config.name)}`, "viper");
            } catch (err) {
      ...
      (async () => {
        ...
        console.log(chalk.blue(`\n` + `DEPLOYING ALL VIPER EVENTS\n`));
        for (const ev of events) {
          try {
            const event = require(join(global.client.mainPath, '../../scripts/events', ev));
            const { config, onLoad, run } = event;
            if (!config || !config.name || !run) {
              global.loading.err(`${chalk.hex('#ff7100')(``)} ${chalk.hex("#FFFF00")(ev)} module is not in the correct format. `, "viper");
              continue;
            }
            ...
            if (global.client.events.has(config.name)) {
              global.loading.err(`${chalk.hex('#ff7100')(``)} ${chalk.hex("#FFFF00")(ev)} module is already deployed.`, "viper");
              continue;
            }
    ...
    console.log(chalk.blue(`\n` + `DEPLOYING VIPER DATA\n`));
    global.loading(`${crayon(``)}deployed ${chalk.blueBright(`${global.client.commands.size}`)} commands and ${chalk.blueBright(`${global.client.events.size}`)} events`, "viper");
    global.loading(`${crayon(``)}deployed time : ${chalk.blueBright(((Date.now() - global.client.timeStart) / 1000).toFixed() + 's')}`, "viper");
    const listenerData = {};
    listenerData.api = loginApiData;
    listenerData.models = botModel;
    const listener = require('../system/listen.js')(listenerData);
    global.custom = require('../../viper.js')({ api: loginApiData });
    global.handleListen = loginApiData.listenMqtt(async (error, message) => {
      if (error) {
        if (error.error === 'Not logged in.') {
          logger("your bot account has been logged out", 'viper');
          return process.exit(1);
        }
        if (error.error === 'Not logged in') {
          logger("your account has been checkpointed, please confirm your account and log in again.", 'viper');
          return process.exit(0);
        }
        console.log(error);
        return process.exit(0);
...
(async () => {
  try {
    ...
    logger(`deployed ${chalk.blueBright('database')} system`, "viper");
    logger(`deploying ${chalk.blueBright('login')} system`, "viper")
    const botData = {};
    botData.models = models
    onBot(botData);
  } catch (error) { logger(`can't deploy ${chalk.blueBright('database')} system`, "viper") }
})();