...
function safeRequire(path, name = "") {
  try {
    ...
    logger.error(`Failed to load ${name || path}: ${e.message}`);
    return null;
  }
}

function safeReadFileSync(path, name = "") {
  try {
    return readFileSync(path, 'utf8');
  } catch (e) {
    logger.error(`Failed to read ${name || path}: ${e.message}`);
    return null;
  }
}
...
if (langRaw) {
  const langLines = langRaw.split(/\r?\n|\r/).filter(l => l && !l.startsWith('#'));
  for (const line of langLines) {
    const [fullKey, ...rest] = line.split("=");
    const value = rest.join("=").replace(/\n/g, "\n");
    const [head, key] = fullKey.split(".");
...
let appState = null;
try {
  const fileData = fs.readFileSync(appStateFile, 'utf8');
