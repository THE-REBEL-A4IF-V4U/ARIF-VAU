...
module.exports.config = { name: "resend", version: "2.0.0", permission: 1, credits: "Nayan", description: "", category: "general", prefix: true, usages: "resend", cooldowns: 0, hide: true, dependencies: { "request": "", "fs-extra": "", "axios": "" } };
...
if (type === "message_unsend") { const logData = global.logMessage.get(messageID); if (!logData) return;
...
module.exports.run = async function ({ api, event, Threads }) { const { threadID, messageID } = event; let threadData = (await Threads.getData(threadID)).data;
