...
module.exports.handleEvent = async ({ api, event, Users }) => {
  ...
  if (banned.includes(senderID)) return; // যদি banned list এ থাকে, কিছু করবো না
  ...
  if (warnData[senderID].count >= settings.warnLimit) {
    ...
    const userName = await Users.getNameUser(senderID) || "ব্যবহারকারী";
    api.sendMessage(`❌ @${userName} তুই খারাপ ছেলে, তোকে আমার সিস্টেম থেকে BAN করলাম!`, threadID, (e, info) => {
      api.markAsReadAll();
    }, { mentions: [{ id: senderID, tag: userName }] });
...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (args[0] === "remove") {
    ...
    const index = badwords.findIndex(word => word === wordToRemove);
  ...
  if (args[0] === "unban") {
    if (args[1]) {
      const index = banned.indexOf(args[1]);
      if (index !== -1) {
