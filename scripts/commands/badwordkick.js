...
module.exports.config = {
  ...
  version: "1.1.0",
  permission: 1,
  credits: "REBEL A4IF V4U (Optimized)",
...
module.exports.handleEvent = async ({ api, event, Users }) => {
  ...
  if (!warnData[senderID]) {
    ...
    if (timePassed < cooldown) return; // Cooldown active, ignore this message

    warnData[senderID].count += 1;
    warnData[senderID].lastWarning = now;
  }

  // 2 warn হলে admin কে notify
  if (warnData[senderID].count === 2 && settings.notifyAdmin && settings.adminID) {
    ...
    api.sendMessage(`⚠️ @${userName} কে ২ বার সতর্ক করা হয়েছে, নজর রাখুন!`, settings.adminID, (e, info) => {
      safeMarkAsReadAll(api);
    }, { mentions: [{ id: senderID, tag: userName }] });
  }

  // warnLimit cross করলে ban করবে
