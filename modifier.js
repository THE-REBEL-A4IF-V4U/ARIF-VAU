...
// লোডিং এনিমেশন function startLoading(message) { const dots = [".", "..", "..."]; let index = 0; process.stdout.write(message); const interval = setInterval(() => { process.stdout.write("\r" + message + dots[index]); index = (index + 1) % dots.length; }, 300); return interval; }
...
async function updatePackageJson() { const data = await fetchJSON( "https://raw.githubusercontent.com/MOHAMMAD-NAYAN/Nayan-Bot/main/package.json", "Updating package.json", "package.json updated successfully.", "Error updating package.json: " ); if (data) { fs.writeFileSync("package.json", JSON.stringify(data, null, 2), "utf-8"); } }

async function fetchVersionFile() { return await fetchJSON( "https://raw.githubusercontent.com/MOHAMMAD-NAYAN/Nayan-Bot/main/version.json", "Loading version file", "Version file loaded successfully.", "Error fetching version.json" ); }
...
const latestPackage = await fetchJSON(
    "https://raw.githubusercontent.com/MOHAMMAD-NAYAN/Nayan-Bot/main/package.json",
